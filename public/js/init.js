...
(function ($) {
  ...
  var Resumo = {
    ...
    modal_prevnext: function modal_prevnext(prevNext, modalBox) {
      var self = this;
      prevNext.find('a').off().on('click', function () {
        ...
        $(".resumo_fn_modalbox .modal_content").stop().animate({
          scrollTop: 0
        }, 500, 'swing');
        self.modal_prevnext(prevNext, modalBox);
        self.imgToSVG();
    ...
    pageWidthAnimation: function pageWidthAnimation() {
      Resumo.changeWidth();
    ...
    movingPlaceholder: function movingPlaceholder() {
    ...
    loadBlogPosts: function loadBlogPosts() {
    ...
    portfolioCarousel: function portfolioCarousel() {
      ...
      $('.resumo_fn_progress_bar').each(function () {
        ...
        pWrap.waypoint({
          handler: function handler() {
            Resumo.progressF(pWrap);
          },
          offset: '90%'
        });
      });
    },
    progressF: function progressF(container) {
      container.find('.progress_item').each(function (i) {
        ...
        setTimeout(function () {
          ...
          percent.html(pValue + '%').css({
            right: 100 - pValue + '%'
          });
        }, i * 500);
      });
    },
    recallProgress: function recallProgress(tabs) {
      tabs.find('.progress_bg').css({
        width: '0%'
      });
      tabs.find('.progress_percent').html('').css({
        right: '100%'
      });
    ...
    rightNav: function rightNav() {
      ...
      opener.on('click', function () {
        ...
        navLi.each(function (i, e) {
          $(e).css({
            transitionDelay: i * speed + transitionTime + 'ms'
          });
        });
        setTimeout(function () {
    ...
    rightPanelScroll: function rightPanelScroll() {
      ...
      nav.css({
        height: navIn.height() - navFooter.outerHeight()
      });
      if ($().niceScroll) {
